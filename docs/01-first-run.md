...
2. Run `./install.sh` from this folder. Answer the three questions. It ends with `doctor`.
...
5. Your agent's first conversation IS the setup interview: it asks about six questions, one
   at a time, writes what it learns into files you can open, and ends by asking for a real
   task. Give it one.
